...
namespace Emgu_Test
{
	...
	public sealed class ListBoxLog : IDisposable
	{
		...
		public ListBoxLog(ListBox listBox, string messageFormat, int maxLinesInListbox)
		{
			...
			ContextMenuStrip menuItems = new ContextMenuStrip();//[] { new ContextMenuStrip("Copy", ) };

			ToolStripDropDownButton fruitToolStripDropDownButton = new ToolStripDropDownButton("Copy", null, new EventHandler(CopyMenuOnClickHandler), "Copy");
			menuItems.Items.Add(fruitToolStripDropDownButton);

			_listBox.ContextMenuStrip = menuItems;
			_listBox.ContextMenuStrip.Click += new EventHandler(CopyMenuPopupHandler);
